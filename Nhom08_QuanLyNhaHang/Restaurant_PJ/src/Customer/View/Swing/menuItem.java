...
public class menuItem extends javax.swing.JPanel {
    ...
    public menuItem(ModelMenu menu, EventMenu event, EventMenuSelected eventSelected, int index) {
        ...
        this.index = index;
        setOpen(false);
        setLayout(new MigLayout("wrap, fillx, insets0", "[fill]", "[fill, 40!]0[fill, 35!]"));
    ...
    protected void paintComponent(Graphics grphcs) {
        ...
        if(menu.getSubMenu().length > 0) {
            createArrawButton(g2);
        }
        super.paintComponent(grphcs);
    }
    
    public void createArrawButton(Graphics2D g2){
        int size = 4;
        int y = 19;
        int x = 205;
